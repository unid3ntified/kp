...
class MscSearch extends Msc
{
    ...
    {
        return [
            [['search', 'msc_name', 'cnid', 'pool', 'non_broadcast_lai', 'null_nri', 'nri_msc', 'spc_msc', 'nb_lai', 'msc_IP_sigtran1', 'msc_IP_sigtran2', 'status', 'remark'], 'safe'],
    ...
    public function search($params)
    {
        ...
        $query->orFilterWhere([
            'cap_value' => $this->search,
            'msc_index' => $this->search,
            'mgw_proxyA_flex' => $this->search,
            'mgw_managerA_circuit' => $this->search,
            'log_date' => $this->search,
        ]);

        $query->orFilterWhere(['like', 'msc_name', $this->search])
            ->orFilterWhere(['like', 'cnid', $this->search])
            ->orFilterWhere(['like', 'pool', $this->search])
            ->orFilterWhere(['like', 'non_broadcast_lai', $this->search])
            ->orFilterWhere(['like', 'null_nri', $this->search])
            ->orFilterWhere(['like', 'nri_msc', $this->search])
            ->orFilterWhere(['like', 'spc_msc', $this->search])
